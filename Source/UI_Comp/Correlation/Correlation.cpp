...
void PhaseCorrelationAnalyserComponent::processBlock(AudioBuffer<float>& buffer)
{
    ...
    for (int i = 0; i < block_size; ++i)
    {
        ...
        {
            sumLL -= SquareQueLL.front();  SquareQueLL.pop();
            sumRR -= SquareQueRR.front();  SquareQueRR.pop();
            sumLR -= SquareQueLR.front();  SquareQueLR.pop();
        }
        ...
        if (++sample_counter >= update_window_samples)
        {
            ...
            const float zeroThresh = 1e-4f;
            if (rmsL < zeroThresh) rmsL = 0.0f;
            if (rmsR < zeroThresh) rmsR = 0.0f;
